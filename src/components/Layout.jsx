...
export default function Layout({ user, activeView, setActiveView, taskStats, onUndo, onRedo, children }) {
  ...
  return (
    <div className="min-h-screen" style={{ background: '#1a1a1a' }}>
      {/* === Top Navbar === */}
      <nav className="glass-card brand-navbar sticky top-0 z-40 border-b border-white/5 pt-safe">
        <div className="px-1 sm:pl-2">
          <div className="flex items-center justify-between h-12 sm:h-10 relative">
            ...
            <div className="flex items-center gap-0.5 sm:gap-1 z-10">
              ...
              <button
                type="button"
                onClick={e => { e.stopPropagation(); setLogoSpinKey(key => key + 1); setShowFile(false); setShowSettings(false); setShowTrash(false) }}
                className="navbar-logo-button"
                title="Logo"
                aria-label="Animate logo"
              >
                <NavLogo spinKey={logoSpinKey} />
              </button>
